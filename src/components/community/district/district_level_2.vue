<template>
  <div>
    <!--1.0 标题-->
    <h2>地区列表 <span class="specOfH4">市级</span></h2>
    ...
    <el-dialog :title="modetext" :visible.sync="dialogFormVisible" :show-close="showX" custom-class="modelStyle">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!--<el-button type="primary" @click="gitEditDatas" v-show="pageId != ''">提交</el-button>-->
      </div>
    </el-dialog>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      gitDatas(){
        if(this.pageId == ''){
          var data = Qs.stringify({name:this.name,parentId:this.$route.query.parentId,level:2});
        }else{
          var data = Qs.stringify({name:this.name,parentId:this.$route.query.parentId,level:2,id:this.pageId});
        }
      ...
      // 查看详情页面
      Viewdetails(id){
        this.$router.push({path: 'district_level_3', query: {parentId: id}});
      },
      // 返回上一页
      toBefore(){
        this.$router.go(-1);
