...
export default {
  ...
  methods: {
    ...
    getData() {
      var data = {
        parklocId: this.$route.query.parkingId
      }
      data = Qs.stringify(data);
      this.$http.post("parkloc/edit", data).then(res => {
        if (res.data.error_code === 2000) {
          console.log(res)
          this.formLabelAlign.parklotName = res.data.data.parklotName//小区名
          this.formLabelAlign.parklotId = res.data.data.parklotId//小区ID
          this.formLabelAlign.phone = res.data.data.phone
          this.formLabelAlign.userId = res.data.data.userId
          this.formLabelAlign.number = res.data.data.number
          this.formLabelAlign.serialNumber = res.data.data.macName
          this.formLabelAlign.lockId = res.data.data.lockId
          this.formLabelAlign.parklotDistrictId = res.data.data.parklotDistrictId
          this.getParkingAreaE(res.data.data.parklotId)
        } else {
          this.$message.error(res.data.error_message)
        }
      }).catch(err => {
    ...
    singleIncrease() {
      if (!this.formLabelAlign.parklotId) {
        ...
        this.$message.error('错了哦，请选择车位编号');
