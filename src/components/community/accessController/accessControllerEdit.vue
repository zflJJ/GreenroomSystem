  ...
  <div class="estate">
    ...
    <el-form label-position="top" label-width="80px" :model="formLabelAlign">
      <el-row :gutter="20">
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <el-form-item label="*门禁类型" prop="intro">
              <el-input v-model="formLabelAlign.intro" placeholder="日常门禁" disabled></el-input>
            </el-form-item>
          </div>
        </el-col>
        <el-col :span="8">
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <el-form-item label="*门禁名称" prop="name">
              <el-input v-model="formLabelAlign.name" placeholder="请输入门禁名称"></el-input>
            </el-form-item>
          </div>
        </el-col>
        <!--<el-col :span="8">-->
        <!--<div class="grid-content bg-purple">-->
        <!--<el-form-item label="*门禁厂商" prop="manufacturer_name">-->
        <!--<el-input v-model="formLabelAlign.manufacturer_name" placeholder="请输入门禁厂商"></el-input>-->
        <!--</el-form-item>-->
        <!--</div>-->
        <!--</el-col>-->
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <el-form-item label="*蓝牙名称" prop="btName">
              <el-input v-model="formLabelAlign.btName" placeholder="蓝牙名称"></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        ...
        <!--<el-col :span="8">-->
        <!--<div class="grid-content bg-purple">-->
        <!--<el-form-item label="*门禁型号" prop="model">-->
        <!--<el-input v-model="formLabelAlign.model" placeholder="门禁型号"></el-input>-->
        <!--</el-form-item>-->
        <!--</div>-->
        <!--</el-col>-->
      </el-row>
      <el-row :gutter="20">
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <el-form-item label="*蓝牙密码" prop="btPwd">
              <el-input v-model="formLabelAlign.btPwd" placeholder="蓝牙密码"></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
  ...
  export default{
    data(){
      var validateName =(rule,value,callback)=>{
        if(value == ""){
          callback(new Error("门禁名称不能为空"));
        }
      };
      var validateBtName=(rule, value, callback)=>{
        if(value == ""){
          callback(new Error('蓝牙名称不能为空'));
        }
      };
      var validatePassword=(rule, value, callback)=>{
        if(value == "" || isNaN(value) || value.length != 6){
          callback(new Error('密码不能为空，并且是6位数的数字'));
        }
      };
      return {
...
//          model:'',
          ...
          parklotId: '',
          parklotName: '',
          id:this.$route.query.accessControllerId
        },
    ...
    methods: {
      //进入获取数据
      getData(){
        this.$http.get("/access/"+ this.$route.query.accessControllerId).then(res => {
          var data = res.data.data;
          console.log(data)
          this.formLabelAlign.name = data.name;
          this.formLabelAlign.btName = data.btName;
          this.formLabelAlign.btPwd = data.btPwd;
      ...
      submitForm(){
        if(this.formLabelAlign.name === ''){
          this.$message.error('错了哦，门禁名称不能为空');
        }else if(this.formLabelAlign.parklotId === ''){
          this.$message.error('错了哦，小区名称不能为空');
        }
//        else if(this.formLabelAlign.manufacturer_name === ''){
//          this.$message.error('错了哦，门禁厂商不能为空');
//        }else if(this.formLabelAlign.model === ''){
//          this.$message.error('错了哦，门禁型号不能为空');
//        }
        else if(this.formLabelAlign.btName === ''){
          this.$message.error('错了哦，蓝牙名称不能为空');
        }else if(this.formLabelAlign.btPwd === ''){
          this.$message.error('错了哦，蓝牙密码不能为空');
        }else{
          var data = Qs.stringify(this.formLabelAlign)
          this.$http.post('access/save', data).then((res) => {
            if (res.data.error_code == 2000) {
            this.$message.success('恭喜你，编辑门禁成功了');
            this.$router.go(-1);
          } else {
