    ...
    <el-dialog
      ...
      <div class="modelSearch">
        <el-form :inline="true"  :model="gatewaySearchFromData">
          <el-form-item label='网关ID'>
            <el-input placeholder="" v-model.trim="gatewaySearchFromData.identifier" @change="clearStart"
  ...
  export default{
    data(){
      return {
        ...
        tempgatewayId:'',//临时存储网关ID,选择车锁类型时存储
      }
    },
    created(){
      this.getData();
    ...
    methods: {
      changeType(val){
        if (val == 1) {
          ...
          this.formLabelAlign.gatewayId = this.tempgatewayId;
        }
      },
      //进入页面获取数据
      getData(){
        ...
        this.tempgatewayId = data.gatewayId;
        if(data.type == 1){
      ...
      submitForm(){
        ...
        if (this.formLabelAlign.btName === 'CrAM_') {
          ...
          this.$message.error('所有*号内容为必填');
        } else if ((this.formLabelAlign.type == "zigbee车锁" || this.formLabelAlign.type == 0 || this.formLabelAlign.type==3 || this.formLabelAlign.type=='Lora车锁')&&( this.formLabelAlign.gatewayId == null  ||this.formLabelAlign.gatewayId == "" )) {
          this.$message.error('所有*号内容为必填');
        } else {
          var type = null;
      ...
      paramsgatewayData(){
        ...
        if (this.gatewaySearchFromData.identifier) datas.identifier = this.gatewaySearchFromData.name;
        this.gatewayTableDatas = [];
        // 开始发送请求
