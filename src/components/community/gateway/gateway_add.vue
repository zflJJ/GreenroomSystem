<template>
  <div>
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <el-form-item label="*网关ID" prop="identifier">
              <el-input v-model="formLabelAlign.identifier" ></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
    ...
    <el-dialog
      ...
      :visible.sync="dialogRouterTable"
      custom-class="modelTableStyle">
      <!--1.0 模态框的搜索区域-->
      <div class="modelSearch">
        <el-form :inline="true"  :model="routerSearchFromData">
          <el-form-item label='路由器名称'>
            <el-input placeholder="请输入路由器名称" v-model.trim="routerSearchFromData.name" @change="clearStart"
                      @keyup.enter.native="routerSearch"></el-input>
          </el-form-item>
          <div class="divStyle">
      ...
      <template>
        <el-table
          style="width: 100%"
          :data="routerTableDatas"
          ref="singleRouterTable"
          row-key="id"
          border>
          <el-table-column
            prop="id"
      ...
      <div slot="footer" class="dialog-footer">
        <div class="page">
          <paingchild :compage="compage" :display="display" :total="total" :pagegroup="pagegroup"
                      v-on:pagechange="pagechangesRouter"></paingchild>
        </div>
        <div class="btnStyle">
...
<script>
  ...
  export default{
    ...
    methods: {
      // 提交数据
      submitForm(){
        if (this.formLabelAlign.identifier === '') {
          ...
          this.$http.post('gateway/save', data).then((res) => {
            ...
            this.$message.success('恭喜你，新增网关成功了');
            this.$router.go(-1);
          } else {
